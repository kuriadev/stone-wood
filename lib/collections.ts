...
export const COLLECTIONS = {
  ...
  menuItems: {
    key: "sw_menu_items",
    load: () => getJson<MenuItem[]>("/api/menu", (j) => j.menuItems as MenuItem[] | undefined),
    sync: idDiff<MenuItem>("/api/menu", (j) => j.item as MenuItem | undefined),
  } satisfies Collection<MenuItem>,
  ...
  bookings: {
    ...
    // Booking ids are generated by the app and are the primary key, so
    // nothing needs reconciling after a create.
    sync: async (prev: Booking[], next: Booking[]) => {
      ...
      for (const b of next) {
        ...
        if (!before) await send("/api/bookings", "POST", b);
