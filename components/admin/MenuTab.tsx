  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div>
          <p style={{ color: C.textXS, fontSize: 10, letterSpacing: 3, marginBottom: 8 }}>FOOD & DRINKS</p>
          <h2 style={{ color: C.textH, fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: mob ? 22 : 26, fontWeight: 400, margin: 0 }}>Menu</h2>
          <p style={{ color: C.textS, fontSize: 12, margin: "6px 0 0" }}>Shown on the public Menu page and offered as a pre-order during Book Now.</p>
        </div>
        <button onClick={openAdd} style={{ ...goldBtn, padding: "10px 20px", fontSize: 11, letterSpacing: 2 }}>+ ADD ITEM</button>
      </div>

      <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
        {(["All", ...CATEGORIES] as const).map((c) => (
          <button key={c} onClick={() => setFilterCat(c)} style={{ padding: "7px 14px", fontSize: 11, borderRadius: 20, cursor: "pointer", background: filterCat === c ? `${gold}18` : "transparent", color: filterCat === c ? gold : C.textS, border: `1px solid ${filterCat === c ? gold + "55" : cBr}` }}>{c}</button>
        ))}
      </div>

      <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr" : "repeat(auto-fill,minmax(260px,1fr))", gap: 16 }}>
        {filtered.map((m) => {
          const sellable = isMenuItemSellable(m, inventory);
          const outOfStock = m.available && !sellable;
          return (
          <div key={m.id} style={{ background: cBg, border: `1px solid ${cBr}`, borderRadius: 10, overflow: "hidden", opacity: sellable ? 1 : 0.55 }}>
            <div style={{ position: "relative" }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={m.img} alt={m.name} style={{ width: "100%", height: 130, objectFit: "cover", display: "block" }} />
              <span style={{ position: "absolute", top: 8, left: 8, background: "rgba(0,0,0,0.55)", color: gold, fontSize: 9, padding: "3px 8px", borderRadius: 20, letterSpacing: 1 }}>{m.category}</span>
              {!m.available && <span style={{ position: "absolute", top: 8, right: 8, background: "rgba(200,60,60,0.85)", color: "#fff", fontSize: 9, padding: "3px 8px", borderRadius: 20, letterSpacing: 1 }}>UNAVAILABLE</span>}
              {outOfStock && <span style={{ position: "absolute", top: 8, right: 8, background: "rgba(245,197,24,0.9)", color: "#000", fontSize: 9, padding: "3px 8px", borderRadius: 20, letterSpacing: 1, fontWeight: 700 }}>SOLD OUT — NO STOCK</span>}
            </div>
            <div style={{ padding: "14px 16px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
                <h4 style={{ color: C.textH, fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: 15, fontWeight: 400, margin: 0 }}>{m.name}</h4>
                <span style={{ color: gold, fontWeight: 700, fontSize: 13, whiteSpace: "nowrap" }}>{fmt(m.price)}</span>
              </div>
              <p style={{ color: C.textS, fontSize: 12, lineHeight: 1.5, marginBottom: 8 }}>{m.desc}</p>
              {m.recipe && m.recipe.length > 0 && (
                <p style={{ color: C.textXS, fontSize: 10, marginBottom: 12 }}>
                  🔗 Linked to {m.recipe.length} ingredient{m.recipe.length > 1 ? "s" : ""} in Inventory
                </p>
              )}
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => toggleAvailable(m)} style={{ ...outBtn, flex: 1, padding: "7px 10px", fontSize: 10, letterSpacing: 1 }}>{m.available ? "MARK OUT" : "MARK AVAILABLE"}</button>
                <button onClick={() => openEdit(m)} style={{ ...outBtn, flex: 1, padding: "7px 10px", fontSize: 10, letterSpacing: 1 }}>EDIT</button>
                <button onClick={() => setConfirmDelete(m)} style={{ background: "rgba(229,85,85,0.06)", color: "#e55", border: "1px solid rgba(229,85,85,0.2)", padding: "7px 10px", fontSize: 10, cursor: "pointer", borderRadius: 6, letterSpacing: 1 }}>DEL</button>
              </div>
            </div>
          </div>
          );
        ...
        {filtered.length === 0 && (
          <p style={{ color: C.textXS, fontSize: 13, gridColumn: "1/-1", textAlign: "center", padding: "32px 0" }}>No items in this category yet.</p>
        )}
      </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.82)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 500, padding: 20, overflowY: "auto" }} role="dialog" aria-modal="true">
          <div style={{ background: isDark ? "linear-gradient(160deg,#0e0c09,#0a0806)" : "#fff", border: `1px solid ${cBr}`, borderRadius: 12, padding: "28px 26px", width: "100%", maxWidth: 440, boxShadow: "0 40px 100px rgba(0,0,0,0.7)", maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 20 }}>
              <div>
                <label style={{ color: gold, fontSize: 10, letterSpacing: 2, display: "block", marginBottom: 6 }}>CATEGORY</label>
              ...
              <div>
                <label style={{ color: gold, fontSize: 10, letterSpacing: 2, display: "block", marginBottom: 6 }}>NAME</label>
                <input value={form.name} onChange={(e) => setF("name", e.target.value)} placeholder="Pork BBQ Skewers" className="sw-input" style={inpS} />
              </div>
              <div>
                <label style={{ color: gold, fontSize: 10, letterSpacing: 2, display: "block", marginBottom: 6 }}>DESCRIPTION</label>
              ...
              <div>
                ...
                {form.img && (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={form.img} alt="" style={{ width: "100%", height: 100, objectFit: "cover", borderRadius: 6, marginTop: 8 }} />
                )}
              </div>
              <div onClick={() => setF("available", !form.available)} style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
                <div style={{ width: 18, height: 18, borderRadius: 4, border: `2px solid ${form.available ? "#4caf50" : cBr}`, background: form.available ? "#4caf50" : "transparent", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {form.available && <span style={{ color: "#fff", fontSize: 11 }}>✓</span>}
                </div>
                <span style={{ color: C.textS, fontSize: 12 }}>Available for order</span>
              </div>
              <div>
                <label style={{ color: gold, fontSize: 10, letterSpacing: 2, display: "block", marginBottom: 6 }}>
                  LINK TO INVENTORY (OPTIONAL)
                </label>
                <p style={{ color: C.textS, fontSize: 11, marginBottom: 8, lineHeight: 1.5 }}>
                  When linked, this item auto-sells-out once any of these ingredients hits zero in Inventory.
                </p>
                {ingredientOpts.length === 0 ? (
                  <p style={{ color: C.textXS, fontSize: 11 }}>No "Food Ingredients" items in Inventory yet — add some there first.</p>
                ) : (
                        ...
                        <div key={ing.id} style={{ display: "flex", alignItems: "center", gap: 8, background: line ? `${gold}14` : "transparent", border: `1px solid ${line ? gold + "55" : cBr}`, borderRadius: 6, padding: "6px 10px" }}>
                          <div onClick={() => toggleRecipeIngredient(ing.id)} style={{ flex: 1, cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>
                            <div style={{ width: 16, height: 16, borderRadius: 4, border: `2px solid ${line ? gold : cBr}`, background: line ? gold : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                              {line && <span style={{ color: "#000", fontSize: 10 }}>✓</span>}
                            </div>
                            <span style={{ color: C.textH, fontSize: 12 }}>{ing.name}</span>
                            <span style={{ color: C.textXS, fontSize: 10 }}>({ing.qty} {ing.unit} in stock)</span>
                          </div>
                          {line && (
                            <div style={{ display: "flex", alignItems: "center", gap: 4, flexShrink: 0 }}>
                              <span style={{ color: C.textS, fontSize: 10 }}>uses</span>
                              <input
                                type="number" min={1} value={line.qtyPerOrder}
                                onChange={(e) => setRecipeQty(ing.id, Number(e.target.value) || 1)}
                                style={{ width: 44, ...inpS, padding: "3px 6px", fontSize: 11 }}
                              />
                              <span style={{ color: C.textS, fontSize: 10 }}>/order</span>
                            </div>
                          )}
                        </div>
                      );
      ...
      {confirmDelete && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.82)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 500, padding: 20 }} role="dialog" aria-modal="true">
          <div style={{ background: isDark ? "#0d0d0d" : "#fff", border: "1px solid rgba(229,85,85,0.3)", borderRadius: 8, padding: "28px 26px", width: "100%", maxWidth: 380 }}>
            <h3 style={{ color: C.textH, fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: 17, fontWeight: 400, marginBottom: 10 }}>Remove "{confirmDelete.name}"?</h3>
            <p style={{ color: C.textS, fontSize: 13, marginBottom: 20 }}>This item will no longer appear on the menu or be orderable.</p>
            <div style={{ display: "flex", gap: 10 }}>
