  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", padding: mob ? "48px 20px" : "80px 24px" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <p className="sw-reveal" style={{ color: gold, letterSpacing: 4, fontSize: 12.5, marginBottom: 10, textAlign: "center" }}>GALLERY</p>
        <h2 className="sw-reveal" style={{ fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: mob ? 28 : 42, color: C.textH, textAlign: "center", marginBottom: 8 }}>A Glimpse of StoneWood</h2>
        <p style={{ color: C.textS, fontSize: 14.5, textAlign: "center", marginBottom: 40 }}>Click any photo to view full size.</p>
        <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr 1fr" : tab ? "repeat(2,1fr)" : "repeat(3,1fr)", gap: mob ? 10 : 14 }}>
          {galleryImgs.map((src, i) => (
            <div key={i} style={{ overflow: "hidden", borderRadius: 8, border: `1px solid ${C.border}`, boxShadow: C.shadowCard, transition: "box-shadow .25s" }}>
              ...
              <img loading="lazy" decoding="async" src={src} srcSet={srcSetFor(src)} sizes={SIZES.tile} alt="" className="sw-img" onClick={() => open(src, i)} style={{ width: "100%", height: mob ? 140 : 240, objectFit: "cover", cursor: "pointer", display: "block" }} />
            </div>
          ))}
        </div>
      </div>
      ...
      {sel && (
        <div onClick={() => setSel(null)} style={{ position: "fixed", inset: 0, background: "rgba(5,4,3,0.96)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 20 }}>
          ...
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img loading="lazy" decoding="async" src={sel} srcSet={srcSetFor(sel)} sizes={SIZES.modal} alt="" onClick={(e) => e.stopPropagation()} style={{ maxWidth: "90%", maxHeight: "88vh", borderRadius: 8, boxShadow: "0 32px 80px rgba(0,0,0,0.8)" }} />
          ...
          <div style={{ position: "fixed", bottom: 24, left: "50%", transform: "translateX(-50%)", color: "rgba(201,168,76,0.5)", fontSize: 13.5, letterSpacing: 2 }}>
